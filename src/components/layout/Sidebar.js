...
import { LayoutDashboard, GraduationCap, Users, FileText, BookOpen, Clock, FlaskConical, Calendar, Briefcase, } from 'lucide-react';
import { useAuth } from '@/context/AuthContext';
export function Sidebar({ isOpen, onClose, variant = 'admin' }) {
    const { user } = useAuth();
    const adminLinks = [
    ...
    return (_jsxs(_Fragment, { children: [isOpen && (_jsx("div", { className: "fixed inset-0 bg-black/50 backdrop-blur-sm z-40 lg:hidden animate-fade-in", onClick: onClose })), _jsxs("aside", { className: `
        ...
        `, children: [_jsxs("div", { className: "p-6 border-b border-primary-dark/30", children: [_jsx("h1", { className: "text-xl font-bold", children: "CCS Profiling" }), _jsx("p", { className: "text-sm text-white/80 mt-1", children: portalName })] }), _jsx("nav", { className: "flex-1 p-4 overflow-y-auto scrollbar-hide", children: _jsx("ul", { className: "space-y-2", children: navLinks.map((link) => (_jsx("li", { children: _jsxs(NavLink, { to: link.to, onClick: onClose, className: ({ isActive }) => `sidebar-nav-item flex items-center gap-3 transition-all duration-200 ${isActive
                                        ? 'sidebar-nav-item-active'
                                        : 'text-white/80 hover:text-white hover:bg-white/10'}`, children: [_jsx(link.icon, { className: "w-5 h-5 flex-shrink-0" }), _jsx("span", { className: "font-medium text-sm", children: link.label })] }) }, link.to))) }) }), _jsxs("div", { className: "p-3 sm:p-4 border-t border-primary-dark/30 bg-gradient-to-t from-primary-dark/20 to-transparent", children: [_jsxs("div", { className: "flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 sm:py-3 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/15 transition-colors", children: [_jsx("div", { className: "w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/20 flex items-center justify-center border border-white/30 flex-shrink-0", children: _jsx("span", { className: "text-xs sm:text-sm font-bold text-white", children: initials }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "text-xs sm:text-sm font-semibold text-white truncate", children: displayName }), _jsx("p", { className: "text-xs text-white/60 truncate", children: roleLabel })] })] }), _jsx("p", { className: "text-xs text-white/50 text-center mt-3 sm:mt-4 font-medium", children: "\u00A9 2026 CCS System" })] })] })] }));
}
