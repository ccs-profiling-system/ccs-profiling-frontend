...
vi.mock('@/services/api/facultyPortalService', () => ({
    default: {
        ...
        getRoster: (...args) => mockGetRoster(...args),
        getAttendance: (...args) => mockGetAttendance(...args),
        submitAttendance: (...args) => mockSubmitAttendance(...args),
    },
}));
const mockFaculty = {
    id: 'fac-1',
...
// ── Arbitraries ───────────────────────────────────────────────────────────────
const rosterStudentArb = fc.record({
    id: fc.uuid(),
    ...
    program: fc.constantFrom('BS Computer Science', 'BS Information Technology', 'BS Information Systems'),
...
function renderAttendancePage() {
    return render(_jsx(MemoryRouter, { children: _jsx(AttendancePage, {}) }));
}
...
describe('Property 12: Attendance page renders a control for every roster student', () => {
    ...
    it('renders an attendance status control for every student in the roster', async () => {
        await fc.assert(fc.asyncProperty(fc.array(rosterStudentArb, { minLength: 1, maxLength: 6 }), async (students) => {
...
describe('Property 13: Attendance data is preserved on submission failure', () => {
     ...
     */
    async function simulateSubmitWithFailure(initialMap, error) {
    ...
    const attendanceMapArb = fc
        .array(fc.record({ id: fc.uuid(), status: statusArb }), { minLength: 1, maxLength: 10 })
        .map((entries) => {
        const map = {};
    ...
    it('attendanceMap is not reset when submitAttendance throws', async () => {
        await fc.assert(fc.asyncProperty(attendanceMapArb, fc.string({ minLength: 1, maxLength: 50 }), async (initialMap, errorMsg) => {
            const { attendanceMap, errorMessage } = await simulateSubmitWithFailure(initialMap, new Error(errorMsg));
