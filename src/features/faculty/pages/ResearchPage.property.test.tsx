...
import type { FacultyPortalProfile, FacultyResearchProject } from '../types';
...
vi.mock('@/services/api/facultyPortalService', () => ({
  default: {
    getResearchProjects: (...args: unknown[]) => mockGetResearchProjects(...args),
...
const researchProjectArb = fc.record<FacultyResearchProject>({
  id: fc.uuid(),
  ...
  status: fc.constantFrom('ongoing', 'completed', 'proposed') as fc.Arbitrary<
    FacultyResearchProject['status']
  >,
...
describe("Property 18: Research page renders all required project fields", () => {
  ...
  it('renders title, description, status, and role for every research project', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(researchProjectArb, { minLength: 1, maxLength: 5 }),
        async (projects) => {
...
describe('Property 26: Research form pre-fills on edit', () => {
  ...
  it('pre-populates all form fields when a project card is clicked for editing', async () => {
    await fc.assert(
      fc.asyncProperty(researchProjectArb, async (project) => {
        ...
        await waitFor(() => {
          const titleInput = screen.getByTestId('research-form-title') as HTMLInputElement;
          ...
          const statusSelect = screen.getByTestId('research-form-status') as HTMLSelectElement;
          expect(statusSelect.value).toBe(project.status);

          const roleSelect = screen.getByTestId('research-form-role') as HTMLSelectElement;
...
describe('Property 27: Research list updates in-place after create or update', () => {
  ...
  it('prepends the new project card after a successful create (list length increases by one)', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(researchProjectArb, { minLength: 0, maxLength: 5 }),
        researchProjectArb,
        async (existingProjects, newProject) => {
          // Ensure the new project id doesn't collide with existing ones
          const safeNewProject: FacultyResearchProject = {
            ...newProject,
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (project, updates) => {
          const updatedProject: FacultyResearchProject = {
            ...project,
            title: updates.title,
