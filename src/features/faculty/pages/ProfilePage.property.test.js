...
const validEmailArb = fc
    .tuple(fc.string({ minLength: 1, maxLength: 10 }).filter((s) => /^[a-z]+$/.test(s)), fc.string({ minLength: 1, maxLength: 10 }).filter((s) => /^[a-z]+$/.test(s)), fc.constantFrom('com', 'edu', 'org', 'ph', 'net'))
    .map(([user, domain, tld]) => `${user}@${domain}.${tld}`);
const facultyProfileArb = fc.record({
    id: fc.uuid(),
...
function renderProfilePage() {
    return render(_jsx(MemoryRouter, { children: _jsx(ProfilePage, {}) }));
}
...
describe('Property 21: Profile form pre-fills with fetched data', () => {
    ...
    it('pre-populates all form fields with values from the fetched profile', async () => {
        await fc.assert(fc.asyncProperty(facultyProfileArb, async (profile) => {
            ...
            await waitFor(() => {
                ...
                const departmentInput = screen.getByTestId('profile-department');
                expect(departmentInput.value).toBe(profile.department);
                const positionInput = screen.getByTestId('profile-position');
                expect(positionInput.value).toBe(profile.position ?? '');
                const specializationInput = screen.getByTestId('profile-specialization');
...
describe('Property 22: Profile update preserves entered values on failure', () => {
     ...
     */
    async function simulateUpdateWithFailure(enteredValues, error) {
    ...
    const profileFormArb = fc.record({
        firstName: nonEmptyAlpha,
        lastName: nonEmptyAlpha,
        email: validEmailArb,
        department: nonEmptyAlpha,
    ...
    it('form values are not reset when updateProfile throws', async () => {
        await fc.assert(fc.asyncProperty(profileFormArb, fc.string({ minLength: 1, maxLength: 50 }), async (enteredValues, errorMsg) => {
            const { form, saveError } = await simulateUpdateWithFailure(enteredValues, new Error(errorMsg));
...
describe('Property 23: Profile email validation blocks invalid addresses', () => {
     ...
     */
    function simulateSubmitWithInvalidEmail(email, updateProfileFn) {
    ...
    // No TLD (no dot after @)
    fc.tuple(fc.string({ minLength: 1, maxLength: 10 }).filter((s) => /^[a-z]+$/.test(s)), fc.string({ minLength: 1, maxLength: 10 }).filter((s) => /^[a-z]+$/.test(s))).map(([user, domain]) => `${user}@${domain}`), 
    // Empty string
    fc.constant(''), 
    // Only whitespace
    fc.constant('   '), 
    ...
    it('validation returns false for invalid email strings', () => {
        fc.assert(fc.property(invalidEmailArb, (invalidEmail) => {
    ...
    it('submit handler does not call updateProfile for invalid email', () => {
        fc.assert(fc.property(invalidEmailArb, (invalidEmail) => {
            ...
            const { blocked, emailError } = simulateSubmitWithInvalidEmail(invalidEmail, updateProfileFn);
