...
vi.mock('@/services/api/facultyPortalService', () => ({
    default: {
        ...
        getMaterials: (...args) => mockGetMaterials(...args),
        uploadMaterial: (...args) => mockUploadMaterial(...args),
        deleteMaterial: (...args) => mockDeleteMaterial(...args),
    },
}));
const mockFaculty = {
    id: 'fac-1',
...
const courseMaterialsArb = fc.array(courseMaterialArb, {
    minLength: 1,
    maxLength: 10,
});
const mockCourse = {
...
function renderMaterialsPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(MaterialsPage, {}) }));
}
...
describe('Property 24: Materials list reflects fetched materials for selected course', () => {
    ...
    it('renders exactly one row per material with all required fields', async () => {
        await fc.assert(fc.asyncProperty(courseMaterialsArb, async (materials) => {
            ...
            const select = screen.getByTestId('materials-course-select');
            ...
            await waitFor(() => {
                ...
                for (const material of materials) {
                    ...
                    expect(downloadLink.href).toContain(material.downloadUrl);
                }
            });
            unmount();
        }), { numRuns: 25 });
...
describe('Property 25: Uploaded material appears in list without reload', () => {
     ...
     */
    function simulateUploadSuccess(existingList, newMaterial) {
        return [newMaterial, ...existingList];
    }
    it('prepended list has length = original + 1 and new item is at index 0', () => {
        fc.assert(fc.property(fc.array(courseMaterialArb, { minLength: 0, maxLength: 20 }), courseMaterialArb, (existingList, newMaterial) => {
