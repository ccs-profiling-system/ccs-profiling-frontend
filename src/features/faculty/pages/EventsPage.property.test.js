...
vi.mock('@/services/api/facultyPortalService', () => ({
    default: {
        ...
        getMyParticipation: (...args) => mockGetMyParticipation(...args),
    },
}));
const mockFaculty = {
    id: 'fac-1',
...
// ── Arbitraries ───────────────────────────────────────────────────────────────
const eventStatusArb = fc.constantFrom('upcoming', 'ongoing', 'completed', 'cancelled');
...
const facultyEventArb = fc.record({
    id: fc.uuid(),
    ...
    description: fc.option(fc.string({ minLength: 5, maxLength: 200 }).filter((s) => s.trim().length > 0), { nil: undefined }),
});
// ── Helper ────────────────────────────────────────────────────────────────────
function renderEventsPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(EventsPage, {}) }));
}
...
describe('Property 19: Events page renders all required event fields', () => {
    ...
    it('renders title, date, time, location, category, and status for every event', async () => {
        await fc.assert(fc.asyncProperty(fc.array(facultyEventArb, { minLength: 1, maxLength: 5 }), async (events) => {
...
describe('Property 20: Event status filter shows only matching events', () => {
    ...
    it('filters events to only those matching the selected status', () => {
        fc.assert(fc.property(eventStatusArb, fc.array(facultyEventArb, { minLength: 0, maxLength: 20 }), (statusFilter, events) => {
...
describe('Property 28: Participation history renders all participation records', () => {
    ...
    const participationStatusArb = fc.constantFrom('registered', 'attended', 'absent');
    ...
    it('renders exactly one row per participation record with title, date, and status', async () => {
        await fc.assert(fc.asyncProperty(fc.array(eventParticipationArb, { minLength: 0, maxLength: 10 }), async (participations) => {
            ...
            await waitFor(() => {
                ...
                // One row per record
                const rows = participations.map((p) => screen.queryByTestId(`participation-row-${p.id}`));
