...
vi.mock('@/services/api/facultyPortalService', () => ({
    default: {
        ...
        getTeachingLoad: (...args) => mockGetTeachingLoad(...args),
...
// ── Arbitraries ───────────────────────────────────────────────────────────────
const facultyCourseArb = fc.record({
    subjectId: fc.uuid(),
...
const teachingLoadArb = fc.record({
...
const dateStringArb = fc
    ...
    .chain((year) => fc.integer({ min: 1, max: 12 }).chain((month) => fc.integer({ min: 1, max: 28 }).map((day) => {
    const mm = String(month).padStart(2, '0');
    const dd = String(day).padStart(2, '0');
    return `${year}-${mm}-${dd}`;
})));
const facultyEventArb = (status) => fc.record({
    id: fc.uuid(),
...
function renderDashboard() {
    return render(_jsx(MemoryRouter, { children: _jsx(FacultyDashboard, {}) }));
}
...
describe('Property 6: Dashboard stat cards reflect data', () => {
    ...
    it('stat cards always reflect the correct course count and upcoming events count', async () => {
        await fc.assert(fc.asyncProperty(fc.array(facultyCourseArb, { minLength: 0, maxLength: 10 }), teachingLoadArb, fc.array(facultyEventArb(), { minLength: 0, maxLength: 15 }), async (courses, teachingLoad, events) => {
...
describe('Property 7: Dashboard course list renders all courses', () => {
    ...
    it('renders a list entry for every course returned by the service', async () => {
        await fc.assert(fc.asyncProperty(fc.array(facultyCourseArb, { minLength: 0, maxLength: 8 }), async (courses) => {
            mockGetCourses.mockResolvedValueOnce(courses);
            ...
            await waitFor(() => {
                for (const course of courses) {
                    expect(screen.getByTestId(`course-item-${course.subjectId}`)).toBeInTheDocument();
                }
            });
            unmount();
        }), { numRuns: 25 });
...
describe('Property 9: Teaching load summary is always displayed', () => {
    ...
    it('always renders total units, total classes, and current semester from the teaching load', async () => {
        await fc.assert(fc.asyncProperty(teachingLoadArb, async (teachingLoad) => {
            mockGetCourses.mockResolvedValueOnce([]);
            mockGetTeachingLoad.mockResolvedValueOnce(teachingLoad);
            mockGetEvents.mockResolvedValueOnce([]);
            const { unmount } = renderDashboard();
            await waitFor(() => {
                expect(screen.getByTestId('teaching-load-units').textContent).toBe(String(teachingLoad.totalUnits));
                expect(screen.getByTestId('teaching-load-classes').textContent).toBe(String(teachingLoad.totalClasses));
                expect(screen.getByTestId('teaching-load-semester').textContent).toBe(teachingLoad.currentSemester);
            });
            unmount();
        }), { numRuns: 25 });
    }, 60000);
