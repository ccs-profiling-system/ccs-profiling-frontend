...
function filterStudents(roster, query) {
    const q = query.toLowerCase();
    return roster.filter((student) => (student.firstName + ' ' + student.lastName).toLowerCase().includes(q) ||
        student.studentId.toLowerCase().includes(q));
}
// ── Helpers ───────────────────────────────────────────────────────────────────
function renderStudentsPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(StudentsPage, {}) }));
}
...
describe('Property 10: Roster search filters by name or ID', () => {
    ...
    it('filtered result contains exactly students matching name or ID (case-insensitive)', () => {
        fc.assert(fc.property(fc.array(rosterStudentArb, { minLength: 0, maxLength: 20 }), fc.string({ minLength: 0, maxLength: 10 }), (roster, query) => {
