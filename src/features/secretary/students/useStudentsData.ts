...
import type { StudentRecord, PaginatedResponse } from '@/types/secretary';
...
export function useStudentsData({ initialPage = 1, initialLimit = 10 }: UseStudentsDataParams = {}) {
  ...
  const fetchStudents = async (page: number, limit: number, searchQuery: string) => {
    try {
      setLoading(true);
      setError(null);
