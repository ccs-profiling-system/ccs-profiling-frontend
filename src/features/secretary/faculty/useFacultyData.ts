...
import type { FacultyRecord } from '@/types/secretary';
import { debounce } from '@/utils/chairHelpers';

interface UseFacultyDataParams {
  initialPage?: number;
  initialLimit?: number;
}

export function useFacultyData({ initialPage = 1, initialLimit = 10 }: UseFacultyDataParams = {}) {
  ...
  const fetchFaculty = async (page: number, limit: number, searchQuery: string) => {
    try {
      setLoading(true);
      setError(null);
