import type { Schedule, CreateSchedulePayload, ConflictResult, ConflictDetail, ScheduleType, CalendarViewMode } from './types';

export const VALID_SCHEDULE_TYPES: ScheduleType[] = ['class', 'exam'];
...
export function validateScheduleForm(payload: Partial<CreateSchedulePayload>): ScheduleFormErrors {
  ...
  if (!payload.type || !VALID_SCHEDULE_TYPES.includes(payload.type as ScheduleType)) {
...
export function detectConflicts(
  payload: Pick<CreateSchedulePayload, 'room' | 'instructor' | 'startTime' | 'endTime'>,
