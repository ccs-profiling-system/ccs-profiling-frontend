...
export function SchedulingAside({ schedules, loading }) {
    ...
    const todaySchedules = useMemo(() => schedules
        .filter((s) => isToday(s.startTime))
        .sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime()), [schedules]);
    const upcomingSchedules = useMemo(() => schedules
