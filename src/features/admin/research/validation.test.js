...
const researchStatusArb = fc.constantFrom('ongoing', 'completed', 'published');
...
describe('Property 1: Invalid research payloads are rejected client-side', () => {
    it('returns errors when title is missing or whitespace-only', () => {
        ...
        fc.stringMatching(/^\s*$/), fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), fc.constantFrom(...VALID_RESEARCH_STATUSES), (title, abstract, category, status) => {
    ...
    it('returns errors when abstract is missing or whitespace-only', () => {
        // Validates: Requirements 1.2
        fc.assert(fc.property(fc.string({ minLength: 1 }), fc.stringMatching(/^\s*$/), fc.string({ minLength: 1 }), fc.constantFrom(...VALID_RESEARCH_STATUSES), (title, abstract, category, status) => {
            const errors = validateResearchForm({
                title,
                abstract,
                category,
    ...
    it('returns errors when category is missing or whitespace-only', () => {
        // Validates: Requirements 1.2
        fc.assert(fc.property(fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), fc.stringMatching(/^\s*$/), fc.constantFrom(...VALID_RESEARCH_STATUSES), (title, abstract, category, status) => {
            const errors = validateResearchForm({
                title,
                abstract,
                category,
    ...
    it('returns errors when status is missing or invalid', () => {
        // Validates: Requirements 1.2
        fc.assert(fc.property(fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), 
        // generate strings that are NOT valid statuses
        fc.string().filter(s => !VALID_RESEARCH_STATUSES.includes(s)), (title, abstract, category, status) => {
            const errors = validateResearchForm({
                title,
                abstract,
                category,
    ...
    it('returns no errors for a fully valid payload', () => {
        ...
        fc.assert(fc.property(fc.string({ minLength: 1 }).filter(s => s.trim().length > 0), fc.string({ minLength: 1 }).filter(s => s.trim().length > 0), fc.string({ minLength: 1 }).filter(s => s.trim().length > 0), fc.constantFrom(...VALID_RESEARCH_STATUSES), (title, abstract, category, status) => {
            const errors = validateResearchForm({
                title,
                abstract,
                category,
...
describe('Property 2: Research status is always a valid enum value', () => {
    it('accepts only ongoing, completed, or published as valid statuses', () => {
        ...
        fc.assert(fc.property(fc.string(), (status) => {
            ...
            const isValid = VALID_RESEARCH_STATUSES.includes(status);
...
describe('Property 5: Status filter returns only matching records', () => {
    it('returns only records whose status matches the filter value', () => {
        ...
        fc.assert(fc.property(fc.array(researchArb, { minLength: 0, maxLength: 20 }), researchStatusArb, (records, status) => {
    ...
    it('returns all records when no status filter is applied', () => {
        ...
        fc.assert(fc.property(fc.array(researchArb, { minLength: 0, maxLength: 20 }), (records) => {
...
describe('Property 6: Category filter returns only matching records', () => {
    it('returns only records whose category matches the filter value', () => {
        ...
        fc.assert(fc.property(fc.array(researchArb, { minLength: 0, maxLength: 20 }), fc.string({ minLength: 1 }).filter(s => s.trim().length > 0), (records, category) => {
...
describe('Property 8: Delete removes entry from list', () => {
    it('after filtering out a record by id, the resulting list does not contain that id', () => {
        ...
        fc.assert(fc.property(fc.array(researchArb, { minLength: 1, maxLength: 20 }), fc.integer({ min: 0, max: 19 }), (records, indexSeed) => {
...
describe('Property 7: Title search returns only matching records (case-insensitive)', () => {
    it('returns only records whose title contains the search string, regardless of case', () => {
        ...
        fc.assert(fc.property(fc.array(researchArb, { minLength: 0, maxLength: 20 }), fc.string({ minLength: 1 }).filter(s => s.trim().length > 0), (records, search) => {
...
describe('Property 3: Status badges are visually distinct per status', () => {
    const allStatuses = ['ongoing', 'completed', 'published'];
...
describe('Property 4: Research details rendering completeness', () => {
    it('rendered details contain title, abstract, category, status, authors, and adviser for any research record', () => {
        ...
        fc.assert(fc.property(researchArb, (record) => {
            const details = renderResearchDetails(record);
            return (details.title === record.title &&
...
describe('Property 11: Files are rendered with name and link', () => {
    ...
    it('every rendered file entry contains the file name and a non-empty URL', () => {
        ...
        fc.assert(fc.property(fc.array(researchFileArb, { minLength: 1, maxLength: 10 }), (files) => {
            const rendered = renderResearchFiles(files);
            return rendered.every((rf, i) => rf.name === files[i].name &&
    ...
    it('rendered file list length matches the input file list length', () => {
        // Validates: Requirements 6.2
        fc.assert(fc.property(fc.array(researchFileArb, { minLength: 0, maxLength: 10 }), (files) => {
            const rendered = renderResearchFiles(files);
...
describe('Property 9: Author assignment round-trip', () => {
    it('authors assigned to a research record can be retrieved without loss', () => {
        ...
        fc.assert(fc.property(fc.array(fc.uuid(), { minLength: 0, maxLength: 10 }), (authorIds) => {
            ...
            return (retrieved.length === authorIds.length &&
    ...
    it('author list with duplicates preserves duplicates', () => {
        // Validates: Requirements 4.4
        fc.assert(fc.property(fc.uuid(), fc.integer({ min: 2, max: 5 }), (authorId, count) => {
...
describe('Property 10: File list delete removes entry', () => {
    ...
    it('after removing a file by id, the resulting list does not contain that id', () => {
        ...
        fc.assert(fc.property(fc.array(researchFileArb, { minLength: 1, maxLength: 10 }), fc.integer({ min: 0, max: 9 }), (files, indexSeed) => {
