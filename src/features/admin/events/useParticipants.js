 ...
 */
export function reconcileAssigned(returned, submittedIds) {
...
export function useParticipants(eventId) {
    ...
    const fetchAvailable = useCallback(async () => {
        ...
        try {
            ...
            const message = err?.response?.data?.message ?? err?.message ?? 'Failed to fetch participants.';
            setState((s) => ({ ...s, loading: false, error: message }));
        }
    }, []);
    const assign = useCallback(async (participantIds) => {
        setState((s) => ({ ...s, loading: true, error: null }));
        try {
            ...
            const message = err?.response?.data?.message ?? err?.message ?? 'Failed to assign participants.';
            setState((s) => ({ ...s, loading: false, error: message }));
            throw err;
        }
    }, [eventId]);
    const remove = useCallback(async (participantId) => {
        setState((s) => ({ ...s, loading: true, error: null }));
        try {
            ...
            const message = err?.response?.data?.message ?? err?.message ?? 'Failed to remove participant.';
            setState((s) => ({ ...s, loading: false, error: message }));
            throw err;
        }
    }, [eventId]);
