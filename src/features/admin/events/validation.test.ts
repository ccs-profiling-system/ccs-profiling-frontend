...
describe('validation.ts — property-based tests', () => {
  ...
  it('Property 2: any payload missing a required field produces a non-empty errors object', () => {
    ...
    const invalidPayload = fc
      .record({
        ...
        type: fc.oneof(blankOrEmpty, nonBlankString) as fc.Arbitrary<any>,
    ...
    fc.assert(
      fc.property(invalidPayload, (payload) => {
  ...
  it('Property 3: only valid EventType strings pass type validation', () => {
    ...
    fc.assert(
      fc.property(fc.string(), (typeStr) => {
        const payload = { title: 'Test', type: typeStr as any, date: '2026-01-01', venue: 'Hall' };
        const errors = validateEventForm(payload);
        const isValid = VALID_EVENT_TYPES.includes(typeStr as any);
        if (isValid) {
  ...
  it('Property 4: only valid EventStatus strings are accepted by resolveDefaultStatus', () => {
    ...
    fc.assert(
      fc.property(fc.string(), (statusStr) => {
        const payload = { title: 'T', type: 'seminar' as any, date: '2026-01-01', venue: 'V', status: statusStr as any };
  ...
  it('Property 9: payloads without a status field always resolve to "upcoming"', () => {
    ...
    fc.assert(
      fc.property(validPayload, (payload) => {
        ...
        const { ...payloadWithoutStatus } = payload as any;
  ...
  it('Property 7: disallowed MIME types are rejected by validateFile', () => {
    ...
    fc.assert(
      fc.property(disallowedMime, (mimeType) => {
  ...
  it('Property 8: files exceeding 10 MB are rejected by validateFile', () => {
    ...
    fc.assert(
      fc.property(oversizedFile, (size) => {
