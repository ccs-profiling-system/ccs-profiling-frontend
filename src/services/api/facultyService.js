...
class FacultyService {
    ...
    async getFacultySubjects(facultyId) {
        return handleRequest(() => api.get(`/admin/faculty/${facultyId}/subjects`).then((r) => r.data));
    }
    async getFacultySkills(facultyId) {
        return handleRequest(() => api.get(`/admin/faculty/${facultyId}/skills`)
            .then((r) => unwrap(r.data).map(mapFacultySkill)));
    }
    async updateFacultySkills(facultyId, skills) {
        return handleRequest(() => api.put(`/admin/faculty/${facultyId}/skills`, { skills })
            .then((r) => unwrap(r.data).map(mapFacultySkill)));
    }
    async getFacultyAffiliations(facultyId) {
        return handleRequest(() => api.get(`/admin/faculty/${facultyId}/affiliations`)
            .then((r) => unwrap(r.data).map(mapFacultyAffiliation)));
    }
    async updateFacultyAffiliations(facultyId, affiliations) {
        return handleRequest(() => api.put(`/admin/faculty/${facultyId}/affiliations`, { affiliations })
            .then((r) => unwrap(r.data).map(mapFacultyAffiliation)));
    }
